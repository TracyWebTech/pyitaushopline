...
class ItauShopline(object):
    ...
    def __init__(self, codigo, chave, **extra):
        ...
        self.urls['boleto'] = extra.get('boleto_url',
                    'https://shopline.itau.com.br/shopline/Itaubloqueto.asp')
        self.urls['consulta'] = extra.get('consulta_url',
                    'https://shopline.itau.com.br/shopline/consulta.asp')
        self.urls['shopline'] = extra.get('shopline_url',
    ...
    def algoritmo(self, token, chave):
        ...
        for j in range(1, len(token)+1):
            ...
            self.indices[l] = i
            caracter = int(ord(token[(j-1):j]) ^ int(
                    self.indices[(self.indices[k] + self.indices[l]) % 256]))
...
if __name__ == '__main__':
    ...
    itaushopline.process(
        pedido=300,
        valor=100.50,
        observacao='',
        nome='GABRIEL GALIASO DE ALMEIDA',
        codigo_inscricao='02',
        numero_inscricao='00000000000',
        cep='14015130',
        endereco='R BERNARDINO DE CAMPOS 1001 SALA 110',
        bairro='HIGIENOPOLIS',
